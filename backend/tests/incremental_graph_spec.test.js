...
describe("Basic operational semantics: invalidate/pull, caching, invalidation", () => {
    ...
    test("outdated propagation", async () => {
        ...
        await expect(g.getFreshness("a")).resolves.toBe("unmaterialized");
        await expect(g.getFreshness("b")).resolves.toBe("unmaterialized");
        await expect(g.getFreshness("c")).resolves.toBe("unmaterialized");
    ...
    test("unchanged optimization long", async () => {
        ...
        await expect(fr("a")).resolves.toBe("unmaterialized");
        await expect(fr("b")).resolves.toBe("unmaterialized");
        await expect(fr("c")).resolves.toBe("unmaterialized");
        await expect(fr("d")).resolves.toBe("unmaterialized");
        await expect(fr("e")).resolves.toBe("unmaterialized");
...
describe("Inspection interface", () => {
    test("getFreshness and listMaterializedNodes behave correctly", async () => {
        ...
        expect(["unmaterialized", "up-to-date", "potentially-outdated"]).toContain(f0);
    ...
    test("pull() on source node materializes it and invalidate() preserves materialization", async () => {
        ...
        // Also verify that the node is persisted in storage (value survives invalidation)
    ...
    test("pull() on leaf node (inputs=[]) must include it in listMaterializedNodes", async () => {
        ...
        // Also verify that the node is persisted in storage
        // This is important for restart resilience
        const storage = makeSemanticStorage(g);
        let freshnessValue;
        await storage.withBatch(async (batch) => {
            freshnessValue = await batch.freshness.get(toJsonKey("leaf"));
        });
        expect(freshnessValue).toBe("up-to-date");
    });
});
