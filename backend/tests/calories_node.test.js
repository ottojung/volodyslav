...
describe("event(e) node", () => {
    test("returns the input text of the identified event", async () => {
        ...
        const result = await iface._incrementalGraph.pull("event", {e: "1"});
    ...
    test("throws EventNotFoundError for an unknown event ID", async () => {
        ...
        await writeEventsToStore(capabilities, [makeEvent("1", "food: a slice of bread")]);
        await expect(
            iface._incrementalGraph.pull("event", {e: "999"})
    ...
    test("reflects updated input text after events change", async () => {
        ...
        const first = await iface._incrementalGraph.pull("event", {e: "1"});
        ...
        const second = await iface._incrementalGraph.pull("event", {e: "1"});
    ...
    test("handles multiple events and returns the correct one", async () => {
        ...
        const r1 = await iface._incrementalGraph.pull("event", {e: "1"});
        const r2 = await iface._incrementalGraph.pull("event", {e: "2"});
        const r3 = await iface._incrementalGraph.pull("event", {e: "3"});
...
describe("calories(e) node", () => {
    test("calls estimateCalories with the target event and raw basic context and returns result", async () => {
        ...
        const result = await iface._incrementalGraph.pull("calories", {e: "1"});
    ...
    test("does not recompute calories when ontology changes", async () => {
        ...
        const first = await iface._incrementalGraph.pull("calories", {e: "1"});
        ...
        const second = await iface._incrementalGraph.pull("calories", {e: "1"});
    ...
    test("uses basic context input rather than only the target event input", async () => {
        ...
        const result = await iface._incrementalGraph.pull("calories", {e: "2"});
    ...
    test("throws EventNotFoundError for an unknown event ID", async () => {
        ...
        await expect(
            iface._incrementalGraph.pull("calories", {e: "999"})
        ).rejects.toThrow("Event with ID 999 not found in all_events");
    ...
    test("computes independently for different event IDs", async () => {
        ...
        const c1 = await iface._incrementalGraph.pull("calories", {e: "1"});
        const c2 = await iface._incrementalGraph.pull("calories", {e: "2"});
        const c3 = await iface._incrementalGraph.pull("calories", {e: "3"});
