...
describe("materialized node invariant", () => {
    ...
    test("rejects stale non-source with every incoming proof present", async () => {
        await buildGraph([
            { output: "source", inputs: [], computor: async () => ({ value: 1 }), isDeterministic: true, hasSideEffects: false },
            { output: "derived", inputs: ["source"], computor: async () => ({ value: 2 }), isDeterministic: true, hasSideEffects: false },
        ]);
        await graph.pull("derived");
        const lookup = db.getActiveIdentifierLookup();
        const derivedId = lookup.keyToId.get('{"head":"derived","args":[]}');
        await db.getSchemaStorage().freshness.put(derivedId, "potentially-outdated");
        await expect(assertValidReplicaMaterializationState(db.getSchemaStorage(), lookup, "test"))
            .rejects.toThrow(/stale but has every incoming validity proof/);
    });

    test("rejects stale node with outgoing validity", async () => {
        await buildGraph([
            { output: "source", inputs: [], computor: async () => ({ value: 1 }), isDeterministic: true, hasSideEffects: false },
            { output: "derived", inputs: ["source"], computor: async () => ({ value: 2 }), isDeterministic: true, hasSideEffects: false },
        ]);
        await graph.pull("derived");
        const lookup = db.getActiveIdentifierLookup();
        const sourceId = lookup.keyToId.get('{"head":"source","args":[]}');
        await db.getSchemaStorage().freshness.put(sourceId, "potentially-outdated");
        await expect(assertValidReplicaMaterializationState(db.getSchemaStorage(), lookup, "test"))
            .rejects.toThrow(/stale but has outgoing validity proofs/);
    });

    test("accepts stale zero-input node with no outgoing validity", async () => {
        await buildGraph([
            { output: "source", inputs: [], computor: async () => ({ value: 1 }), isDeterministic: true, hasSideEffects: false },
        ]);
        await graph.pull("source");
    ...
    test("accepts stale non-source missing one causal proof", async () => {
        await buildGraph([
            { output: "source", inputs: [], computor: async () => ({ value: 1 }), isDeterministic: true, hasSideEffects: false },
            { output: "derived", inputs: ["source"], computor: async () => ({ value: 2 }), isDeterministic: true, hasSideEffects: false },
        ]);
