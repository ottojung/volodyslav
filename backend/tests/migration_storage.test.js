...
async function setupStandardGraph(storage, newHeadIndex, opts = {}) {
    ...
    return makeMigrationStorage(storage, newHeadIndex, [A, B, C, D], "testfingerprinta", 0, scheme, scheme, lookup);
}
...
describe("MigrationStorage", () => {
    ...
    describe("Section 1: Decision idempotency & conflicts", () => {
        test("keep(A) twice succeeds (idempotent)", async () => {
            ...
            await storage.freshness.put(A, "up-to-date");
            await storage.timestamps.put(A, { createdAt: "2024-01-01T00:00:00.000Z", modifiedAt: "2024-01-01T00:00:00.000Z" });
            const ms = makeMigrationStorage(storage, headIndex, [A], "testfingerprinta", 0, scheme, scheme, lookup);
    ...
    describe("Section 5: DELETE propagation", () => {
        ...
        test("delete propagation uses structural inputs when valid flags are missing (stale dependent)", async () => {
            ...
            const ms = makeMigrationStorage(storage, headIndex, [A, B], "testfingerprinta", 0, scheme, scheme, lookup);
        ...
        test("fan-in deletion uses structural inputs when valid flags are missing (stale dependent)", async () => {
            ...
            const ms = makeMigrationStorage(storage, headIndex, [B, C, D], "testfingerprinta", 0, scheme, scheme, lookup);
        ...
        test("delete propagation replaces automatically propagated invalidation", async () => {
            ...
            await storage.values.put(D, DUMMY_VALUE);
            await storage.valid.put(C, [D]);
            const ms = makeMigrationStorage(storage, headIndex, [A, C, D], "testfingerprinta", 0, scheme, scheme, lookup);
        ...
        test("delete propagation follows removed target dependency", async () => {
            ...
            await storage.values.put(A, DUMMY_VALUE);
            await storage.values.put(B, DUMMY_VALUE);
            const ms = makeMigrationStorage(storage, headIndex, [A, B], "testfingerprinta", 0, oldScheme, newScheme, lookup);
        ...
        test("delete() succeeds for node removed from target schema", async () => {
            ...
            const ms = makeMigrationStorage(storage, headIndex, [legacy], "testfingerprinta", 0, oldScheme, newScheme, lookup);
        ...
        test("created dependent of deleted input conflicts during finalization", async () => {
            ...
            const lookup = makeLookupFromKeys([A]);
            await storage.values.put(A, DUMMY_VALUE);
            const ms = makeMigrationStorage(storage, headIndex, [A], "testfingerprinta", 0, oldScheme, newScheme, lookup);
    ...
    describe("Section 8: Schema compatibility checks", () => {
        ...
        test("keep() fails hard when lookup cannot resolve node", async () => {
            const storage = makeInMemorySchemaStorage();
            const headIndex = makeHeadIndex(["A", "B", "C", "D"]);
            const A = nk("A");
            const scheme = makeStandardGraphScheme();
            // Create lookup missing A — its semantic key cannot be resolved.
            const lookup = makeLookupFromKeys([nk("B"), nk("C"), nk("D")]);
            await storage.values.put(A, DUMMY_VALUE);
            ...
            const ms = makeMigrationStorage(storage, headIndex, [A, nk("B"), nk("C"), nk("D")], "testfingerprinta", 0, scheme, scheme, lookup);
            const err = await ms.keep(A).catch((e) => e);
            expect(isSchemaCompatibility(err)).toBe(true);
        });
    ...
    describe("Section 9: create() method", () => {
        ...
        test("create() on a node whose key exists in identifiers_keys_map throws CreateExistingNodeError", async () => {
            ...
            const NEW = nk("NEW");
            const scheme = makeZeroInputScheme(["A", "NEW"]);
            // Lookup must include NEW so the create collision check finds it.
            const lookup = makeLookupFromKeys([A, NEW]);
            await storage.values.put(A, DUMMY_VALUE);
            await storage.freshness.put(A, "up-to-date");
            await storage.timestamps.put(A, { createdAt: "2024-01-01T00:00:00.000Z", modifiedAt: "2024-01-01T00:00:00.000Z" });
            const ms = makeMigrationStorage(storage, headIndex, [A], "testfingerprinta", 0, scheme, scheme, lookup);
        ...
        test("create() produces deterministic identifiers across identical runs", async () => {
            ...
            const ms1 = makeMigrationStorage(storage1, headIndex1, [A1], "testfingerprinta", 0, scheme1, scheme1, lookup1);
            await ms1.keep(A1);
            ...
            const ms2 = makeMigrationStorage(storage2, headIndex2, [A2], "testfingerprinta", 0, scheme2, scheme2, lookup2);
            await ms2.keep(A2);
        ...
        test("create() produces deterministic identifiers regardless of identifiers_keys_map", async () => {
            ...
            const msA = makeMigrationStorage(storageA, headIndexA, [A_A], "testfingerprinta", 0, schemeA, schemeA, lookupA);
            await msA.keep(A_A);
            ...
            const msB = makeMigrationStorage(storageB, headIndexB, [A_B, B_B], "testfingerprinta", 0, schemeB, schemeB, lookupB);
            await msB.keep(A_B);
            await msB.keep(B_B);
