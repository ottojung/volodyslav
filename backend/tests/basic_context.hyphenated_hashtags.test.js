...
describe("basic context with hyphenated hashtags", () => {
    ...
    test("computeBasicContextForEventId keeps only exact serialized matches 3", () => {
        const serializedEvents = [
            makeSerializedEvent("target", "food [when 0 hours ago] One bottle of yogurt. 100 grams total. Detailed unavailable. Small cup of mixed nuts. About 30 grams total. See photo #mixed-nuts-3."),
            makeSerializedEvent("prefix-only", "text different yogurt #yogurt-yoplait2"),
            makeSerializedEvent("prefix-only2", "text different yogurt #yogurt-yoplait"),
            makeSerializedEvent("unrelated", "text unrelated #unrelated"),
            makeSerializedEvent("unrelated2", "photo unrelated #unrelated2"),
        ...
        expect(basicContext.events.map((event) => event.id)).toEqual(["target", "exact2", "exact3"]);
    });
});
