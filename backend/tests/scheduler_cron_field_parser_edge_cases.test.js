...
describe("scheduler cron field parser edge cases", () => {
    ...
    describe("slash syntax rejection (no longer supported)", () => {
        test("should parse valid explicit lists that were formerly step expressions", () => {
            expect(parseField("0,5,10,15,20,25,30,35,40,45,50,55", FIELD_CONFIGS.minute)).toEqual(createMask([0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55], 59));
            expect(parseField("0,2,4,6,8,10", FIELD_CONFIGS.minute)).toEqual(createMask([0, 2, 4, 6, 8, 10], 59));
            expect(parseField("1,4,7", FIELD_CONFIGS.day)).toEqual(createMask([1, 4, 7], 31));
        });

        test("should throw FieldParseError for slash characters (no longer supported)", () => {
            expect(() => parseField("/5", FIELD_CONFIGS.minute)).toThrow();
            expect(() => parseField("*/5", FIELD_CONFIGS.minute)).toThrow();
        ...
        test("should throw FieldParseError for any remaining slash characters", () => {
            expect(() => parseField("*/0", FIELD_CONFIGS.minute)).toThrow();
            expect(() => parseField("*/-1", FIELD_CONFIGS.minute)).toThrow();
            expect(() => parseField("*/abc", FIELD_CONFIGS.minute)).toThrow();
            // Note: slash syntax is no longer supported at all
        });

        test("should handle step values larger than range", () => {
            // This test is no longer relevant as step syntax is removed
            // Instead test that slash syntax is rejected
