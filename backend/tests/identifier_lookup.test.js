...
const ID_A = id("1-abcdefghijklmnop");
const ID_B = id("2-abcdefghijklmnop");
const ID_C = id("3-abcdefghijklmnop");
const ID_D = id("4-abcdefghijklmnop");
...
describe("makeIdentifierLookup", () => {
    test("builds lookup from valid sorted entries", () => {
        ...
        expect(nodeIdentifierToString(requireNodeIdentifierForKey(lookup, KEY_X))).toBe("1-abcdefghijklmnop");
        expect(nodeIdentifierToString(requireNodeIdentifierForKey(lookup, KEY_Y))).toBe("2-abcdefghijklmnop");
    });
    ...
    test("builds lookup from unsorted entries (serialized cache is sorted)", () => {
        ...
        expect(nodeIdentifierToString(serialized[0][0])).toBe("1-abcdefghijklmnop");
        expect(nodeIdentifierToString(serialized[1][0])).toBe("2-abcdefghijklmnop");
    });
...
describe("serializeIdentifierLookup", () => {
    test("returns sorted entries", () => {
        ...
        expect(nodeIdentifierToString(result[0][0])).toBe("1-abcdefghijklmnop");
        expect(nodeIdentifierToString(result[1][0])).toBe("2-abcdefghijklmnop");
...
describe("nodeKeyToIdFromLookup / nodeIdToKeyFromLookup", () => {
    test("returns identifier for existing key", () => {
        ...
        expect(nodeIdentifierToString(nodeKeyToIdFromLookup(lookup, KEY_X))).toBe("1-abcdefghijklmnop");
    });
...
describe("allocateNodeIdentifier", () => {
    test("allocates new identifier", () => {
        ...
        expect(nodeIdentifierToString(result)).toBe("1-abcdefghijklmnop");
...
describe("cloneIdentifierLookup", () => {
    test("clone has same entries", () => {
        ...
        expect(nodeIdentifierToString(requireNodeIdentifierForKey(clone, KEY_X))).toBe("1-abcdefghijklmnop");
        expect(nodeIdentifierToString(requireNodeIdentifierForKey(clone, KEY_Y))).toBe("2-abcdefghijklmnop");
    });
...
describe("mergeIdentifierLookups", () => {
    test("merges new entries from overlay into base", () => {
        ...
        expect(nodeIdentifierToString(requireNodeIdentifierForKey(base, KEY_X))).toBe("1-abcdefghijklmnop");
    });
    ...
    test("updates serialized cache after merge", () => {
        ...
        expect(nodeIdentifierToString(base.serialized[0][0])).toBe("1-abcdefghijklmnop");
        expect(nodeIdentifierToString(base.serialized[1][0])).toBe("2-abcdefghijklmnop");
    });
    ...
    test("serialized preserves sort after multiple merges", () => {
        ...
        expect(nodeIdentifierToString(base.serialized[0][0])).toBe("1-abcdefghijklmnop");
        expect(nodeIdentifierToString(base.serialized[1][0])).toBe("2-abcdefghijklmnop");
        expect(nodeIdentifierToString(base.serialized[2][0])).toBe("4-abcdefghijklmnop");
    });
});
...
describe("txNodeKeyToId / txNodeIdToKey", () => {
    test("returns base entry when overlay is empty", () => {
        const base = makeIdentifierLookup([[ID_A, KEY_X]]);
        const txLookup = makeTransactionIdentifierLookup(base);
        expect(nodeIdentifierToString(txNodeKeyToId(txLookup, KEY_X))).toBe("1-abcdefghijklmnop");
    ...
    test("overlay shadows base", () => {
        ...
        txLookup.idToKey.set("2-abcdefghijklmnop", KEY_X);
        expect(nodeIdentifierToString(txNodeKeyToId(txLookup, KEY_X))).toBe("2-abcdefghijklmnop");
...
describe("txAllocateNodeIdentifier", () => {
    ...
    test("allocates new identifier through root database", () => {
        ...
        expect(nodeIdentifierToString(result)).toBe("2-abcdefghijklmnop");
        expect(nodeIdentifierToString(txNodeKeyToId(txLookup, KEY_X))).toBe("2-abcdefghijklmnop");
    ...
    test("reuses identifier already in overlay", () => {
        ...
        expect(nodeIdentifierToString(second)).toBe("1-abcdefghijklmnop");
    });
...
describe("serializeTransactionLookup", () => {
    ...
    test("includes new entries from overlay", () => {
        const base = makeEmptyIdentifierLookup();
        const txLookup = makeTransactionIdentifierLookup(base);
        txLookup.idToKey.set("1-abcdefghijklmnop", KEY_X);
        const result = serializeTransactionLookup(txLookup);
        expect(result.length).toBe(1);
        expect(nodeIdentifierToString(result[0][0])).toBe("1-abcdefghijklmnop");
        expect(keyStr(result[0][1])).toBe("key_x");
...
describe("commitTransactionLookup", () => {
    ...
    test("multiple commits accumulate correctly", () => {
        ...
        expect(nodeIdentifierToString(requireNodeIdentifierForKey(base, KEY_X))).toBe("1-abcdefghijklmnop");
        expect(nodeIdentifierToString(requireNodeIdentifierForKey(base, KEY_Y))).toBe("2-abcdefghijklmnop");
    });
});
...
describe("Serialization roundtrip", () => {
    test("serialize -> makeIdentifierLookup preserves identity", () => {
        ...
        expect(nodeIdentifierToString(requireNodeIdentifierForKey(restored, KEY_X))).toBe("1-abcdefghijklmnop");
        expect(nodeIdentifierToString(requireNodeIdentifierForKey(restored, KEY_Y))).toBe("2-abcdefghijklmnop");
    ...
    test("transaction lifecycle: allocate, serialize, commit, serialize again", () => {
        ...
        expect(nodeIdentifierToString(beforeCommit[0][0])).toBe("1-abcdefghijklmnop");
        ...
        expect(nodeIdentifierToString(afterCommit[0][0])).toBe("1-abcdefghijklmnop");
