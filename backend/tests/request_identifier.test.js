...
const fs = require('fs');
const { fromRequest, makeDirectory, markDone, isDone } = require('../src/request_identifier');
...
describe('Request Identifier', () => {
    describe('fromRequest', () => {
        ...
        it('throws error when request_identifier is missing', () => {
            const req = { query: {} };
            expect(() => fromRequest(req)).toThrow('Missing request_identifier field');
        });
    ...
    describe('markDone and isDone', () => {
        it('creates and checks done marker', async () => {
            ...
            expect(isDone(reqId)).toBe(false);
            
            await markDone(reqId);
            
            expect(isDone(reqId)).toBe(true);
            expect(fs.existsSync(path.join(uploadDir, 'test123.done'))).toBe(true);
        });
