...
const { toEpochMs, fromEpochMs, fromHours, fromMinutes, fromDays, fromObject, fromISOString } = require("../src/datetime");
...
describe("scheduler stories", () => {
    ...
    test("should verify time consistency across scheduler operations", async () => {
        ...
        timeControl.advanceByDuration(fromObject({minutes: 45})); // 45 minutes to reach 01:00:00
        await schedulerControl.waitForNextCycleEnd();
    ...
    test("should handle long-term scheduler behavior with mixed success and failure rates", async () => {
        ...
        timeControl.setDateTime(fromEpochMs(startTime));
        schedulerControl.setPollingInterval(fromObject({milliseconds: 100}));
    ...
    test("should maintain exact execution counts across scheduler restart", async () => {
        ...
        const startDateTime = fromISOString("2021-01-01T14:00:00.000Z");
        timeControl.setDateTime(startDateTime);
        schedulerControl.setPollingInterval(fromObject({milliseconds: 1}));
        ...
        newTimeControl.setDateTime(startDateTime.advance(fromHours(2)));
        newSchedulerControl.setPollingInterval(fromObject({milliseconds: 1}));
    ...
    test("should not block executions when a slow task is running", async () => {
        ...
        timeControl.advanceByDuration(fromObject({minutes: 45})); // 45 minutes to reach 02:00:00
        await schedulerControl.waitForNextCycleEnd();
        // Wait some time.
        await new Promise(resolve => setTimeout(resolve, 200));
