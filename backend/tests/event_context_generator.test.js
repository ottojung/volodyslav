...
describe("generators/individual/event_context", () => {
    ...
    describe("computeEventContextsIncremental()", () => {
        const { computeEventContextsIncremental } = require("../src/generators/individual/event_context");

        test("processes only new entries on subsequent calls", () => {
            // Initial computation
            const metaEvents1 = [
                {
                    action: "add",
            ...
            const result1 = computeEventContextsIncremental(metaEvents1, null);
            expect(result1.contexts).toHaveLength(1);
            expect(result1.state.lastProcessedLength).toBe(1);
            ...
            const result2 = computeEventContextsIncremental(metaEvents2, result1.state);
            expect(result2.contexts).toHaveLength(2);
            expect(result2.state.lastProcessedLength).toBe(2);

            // Both events should have each other in context (shared #project)
            const event1Context = result2.contexts.find(ctx => ctx.eventId === "1");
            const event2Context = result2.contexts.find(ctx => ctx.eventId === "2");
            expect(event1Context.context).toHaveLength(2);
            expect(event2Context.context).toHaveLength(2);
        });

        test("handles delete actions incrementally", () => {
            // Initial state with two events
            const metaEvents1 = [
                {
                    action: "add",
                    event: {
            ...
            const result1 = computeEventContextsIncremental(metaEvents1, null);
            expect(result1.contexts).toHaveLength(2);
            ...
            const result2 = computeEventContextsIncremental(metaEvents2, result1.state);
            expect(result2.contexts).toHaveLength(1);
            expect(result2.contexts[0].eventId).toBe("1");
            expect(result2.contexts[0].context).toHaveLength(1); // Only itself
        });

        test("returns unchanged contexts when no new entries", () => {
            const metaEvents = [
                {
                    action: "add",
                    event: {
            ...
            const result1 = computeEventContextsIncremental(metaEvents, null);
            const result2 = computeEventContextsIncremental(metaEvents, result1.state);
            
            expect(result2.contexts).toEqual(result1.contexts);
            expect(result2.state.lastProcessedLength).toBe(1);
        });
    });
});
