...
/**
 * Structural database shape used by the legacy semantic-key storage path.
 * Test doubles in older unit tests often implement `putOp`/`delOp` without
 * the newer `rawPutOp` helper, so this path must support both.
 * @typedef {object} LegacyBatchDatabase
 * @property {(key: NodeKeyString, value: *) => *} putOp
 * @property {((key: NodeKeyString, value: *) => *) | undefined} [rawPutOp]
 * @property {(key: NodeKeyString) => *} delOp
 * @property {(key: NodeKeyString) => Promise<* | undefined>} get
 */
...
function makeLegacyGraphStorage(schemaStorage) {
    ...
    const withBatch = async (fn) => {
         ...
         * @template TValue
         * @param {string} tag
         * @param {LegacyBatchDatabase} database
