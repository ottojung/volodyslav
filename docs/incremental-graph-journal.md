...
Let `n` be represented current/historic semantic keys, `r` durable authors, and `c` losslessly retained full `(key,receiverValueOrigin,sourceGeneration,sourceValueOrigin)` reset correspondences. Public-action and causal evidence use `O(nr²)`. Each exact correspondence carrier includes an `O(r)` causal observation vector, so correspondence storage is `O(cr)`; coverage is `O(r)`. The global bound is therefore `O(nr²+cr+r)`. The `cr` term is necessary when lagging explicitly certified origins must remain recognizable. For `n=0,c=0`, the journal is empty while coverage may be `O(r)`. Application-owned filter-bound cursor strings are not database storage.
