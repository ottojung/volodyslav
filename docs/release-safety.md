...
Many operations update several records that only make sense together. In the incremental graph, for example, values, freshness, inputs, validity sets, timestamps, and semantic-key-to-identifier mappings together describe one logical graph state. Publishing only some of those records can create a database that is syntactically readable but semantically inconsistent.
