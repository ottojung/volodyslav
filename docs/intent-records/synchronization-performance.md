...
GitHub issue [#1607](https://github.com/ottojung/volodyslav/issues/1607), "Make incremental synchronization cost depend on changes, not graph size", owns the future change-sensitive synchronization performance contract. That issue includes receiver validation, materialized-view construction/cutover, identifier/index maintenance, journal processing, and other lifecycle work which could quietly reintroduce graph-sized work.

Until #1607 is resolved and this intent is superseded or updated, correctness-oriented Journal 3 specifications may permit whole-replica work even when only a small journal suffix is new.

This deferral concerns running time only. It does not weaken synchronization correctness, convergence, replay correctness, or independently stated streaming requirements.
...
This requirement does not imply the deferred end-to-end running-time bound from `$id-3572255392439745` and does not prohibit whole-replica validation or other graph-sized work for correctness.
