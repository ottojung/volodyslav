...
Let G denote the size of the current materialized IncrementalGraph state plus current-state metadata whose size is bounded by that graph; G does **not** include retained Journal history or history-proportional Journal indexes. Let H denote the size of retained authoritative Journal history. Journal-specific routine startup/open work must be O(1 + G) and must be independent of H. In particular, routine open must not perform Θ(H) full-history well-formedness validation, replay-equality checking, history-index validation, or reconstruction solely because old retained history exists.
...
This bound applies only to routine opening of an already-current supported database. Explicit migration, synchronization/import, bootstrap, absent restoration, maintenance cutover, and explicit projection rebuild/maintenance may perform transformation, source-transfer, or reconstruction work over retained/transferred state as required by their own contracts. This does **not** relax the historical-validation complexity classes in `$id-6845129073418625`: migration alone may validate O(H), synchronization/reset validate O(C), and every other normal validation path is O(1) in H. If routine open finds metadata inconsistent with a supported committed state, it may fail or enter an explicitly requested maintenance/rebuild transition; it must not silently fall back to a full-history scan as the normal open path.
...
Across any collection of supported database states that can coexist or later be combined through supported operations, a given `NodeIdentifier` must identify the same node materialization lineage wherever it appears.

Two distinct materialization lineages must not use the same `NodeIdentifier` within such a collection of states.
