...
* $\texttt{RegistrationList}$ — a finite ordered list $R = [x_1,\dots,x_{n}]$ of tasks. Indexing uses $R[i]$ for $1 \le i \leq n$ and strong list membership $x \in_{\text{list}} R \iff \exists i.\; R[i] = x$. Duplicate tasks and duplicate task identifiers are permitted.
