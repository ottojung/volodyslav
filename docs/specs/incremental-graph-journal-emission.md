...
Its `absorbsThrough` vector is the closed per-author prefix intentionally consumed. `applicableLineages(J,K)` selects current presence and fallback authority; it is not the controlled-reset source-consumption boundary. Controlled reset carries every retained future-relevant source-anchor absorption vector for the NodeKey, including a currently inapplicable causal-maximal assertion. Thus a carrier with `absorbsThrough[B]=100` keeps delayed B:90 absorbed without importing source journal or `journalCoverage`.
