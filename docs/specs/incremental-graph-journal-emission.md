...
Archive installation is independently committable when every graph, journal-authority, proof, correspondence, lineage, and causal obligation is already represented. Such a reset authors no event and changes neither `localJournalCounter` nor timestamps. The next identical reset is silent because its complete resulting archive is already installed.
