...
Its `absorbsThrough` vector is the closed per-author prefix intentionally consumed. `applicableAnchors(J,K)` is exactly the per-anchor displacement test defined by the journal synchronization specification; only those groups contribute to `applicableCut` and fallback authority. It is not the controlled-reset source-consumption boundary. Controlled reset carries every retained future-relevant source-anchor absorption vector for the NodeKey, including a currently inapplicable causal-maximal assertion. Thus a carrier with `absorbsThrough[B]=100` keeps delayed B:90 absorbed without importing source journal or `journalCoverage`.
