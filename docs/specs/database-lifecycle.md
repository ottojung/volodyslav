...
For every reset-touched NodeKey, reset retains one compact `ResetLineage(consumedThrough,consumedGeneration,consumedValueOrigin)` on receiver authority. A present target stores it on the receiver freshness assertion anchored to the retained value origin; an absent target stores it on the real public delete and uses null source IDs. `consumedThrough` componentwise carries every prior lineage vector for the receiver anchor and incorporates closed receiver/source prefixes newly inspected by this reset. If the receiver already has the exact requested semantic target, a contiguous coordinate suffix consisting only of that reset anchor and lineage carrier is bookkeeping and is not chased; any later genuine receiver/source history advances the coordinate. A different target generation/origin never receives that exemption. Reset does not install those coordinates in receiver journalCoverage.

For each author A, an event at or below `consumedThrough[A]` was observed and is absorbed; a same-lineage scoped event, delete, or rematerialization above it remains live. Missing A means zero. Event authorship, not the physical carrier or reset-source container, selects the coordinate. A later summary on the same receiver anchor carries the earlier vector componentwise and therefore subsumes earlier exact source origins inside that consumed prefix. Present exact correspondence still records the semantic `isEqual` copy needed for hard unsupported-cache absorption; absent lineage needs no value correspondence.
