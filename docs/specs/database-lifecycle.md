...
`DatabaseFingerprint`,
journal entries, `journalCoverage`, `localJournalCounter`, and `causalSummary`. Absent-state
...
Idempotence compares the hypothetical combined frontier, future-relevant absorption/context, and exact correspondence, not raw coverage growth. If existing authority already represents unchanged S0 and the graph target matches, reset authors nothing and leaves graph, journal, receiver coverage, local counter, causal summary, identifiers, and timestamps byte-for-byte unchanged.
