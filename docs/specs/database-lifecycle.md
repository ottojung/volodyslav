...
Reset planning enumerates every future-relevant receiver and source tagged anchor it consumes and computes that anchor's effective cut from both retained same-anchor carriers and the snapshot's `resetAnchorCuts` entry. The new lineage componentwise carries every such effective cut, validated source coverage, and every genuine retained public event coordinate consumed for this NodeKey. A coordinate present only in compact summary state is therefore preserved. This transfers semantic absorption only—exact semantic correspondence remains independently scoped and is never widened from the vector. Reset-of-reset consumes the source snapshot's durable reset meaning without importing its journal, cut-summary records, or `journalCoverage`; the effective coordinates move into the new receiver-authored lineage.
...
Idempotence compares the hypothetical combined frontier, every consumed effective anchor cut, future-relevant context, and exact correspondence, not raw coverage growth. If existing authority already represents unchanged S0 and the graph target matches, reset authors nothing and leaves graph, journal, reset-anchor cut summaries, receiver coverage, local counter, causal summary, identifiers, and timestamps byte-for-byte unchanged.
