...
For every reset-touched NodeKey, reset retains one compact `ResetLineage(consumedThrough,correspondence)` on receiver authority. A present target stores it on the receiver freshness assertion anchored to the retained value origin; present-to-absent stores it on the real public delete. Absent-to-absent authors an internal reset-observation anchored to the existing delete or explicit absence, with no public action. `consumedThrough` componentwise carries every prior lineage vector for the receiver anchor, the validated source coverage, and every retained public event coordinate for this NodeKey. A lineage carrier and the exact receiver generation/value/delete anchor it names are explicitly reset bookkeeping for that anchor and are the only coordinates excluded on an unchanged repeat. Compaction retains the public coordinate maxima, so genuine delete/rematerialize/edit history remains distinguishable after restart. Bookkeeping on another key or anchor is never an exemption. A changed target includes its former anchor/carrier as consumed history. Reset does not install those coordinates in receiver journalCoverage.
