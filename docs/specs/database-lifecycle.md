...
- retained same-author/key/generation/value-origin validation vectors are componentwise monotone when comparable evidence is present;
...
For each retained source value copied or semantically matched by reset, a receiver freshness assertion carries a compact `resetCorrespondence(receiverValueOrigin, sourceAuthor, consumedThrough, consumedSourceGeneration, consumedSourceValueOrigin)`. Reset authors one when no retained summary for that receiver origin/source author subsumes the consumed prefix and exact semantic correspondence, including equal-valued hard multi-input state with different revisions. The certificate records an observed `isEqual` semantic copy and a complete source-author cutoff; it neither imports source journal/coverage nor makes arbitrary unsupported caches spreadable. An unchanged reset finds the same summary and is silent. A newer summary may replace an older same-source summary only after observing the intervening complete prefix and the same receiver semantic value.
...
Separate old partial validations are not combined; the reset validation is one new assertion justified by its actual joint observation. Unseen/concurrent history above the consumed prefixes remains live. A delayed compacted event within a claimed prefix is cleared. Presence and value reconciliation interpret the correspondence as a lineage bridge: events by `sourceAuthor` in the consumed lineage at or below `consumedThrough` are absorbed, whereas a later scoped event, delete, or rematerialization above that cutoff is eligible under ordinary semantics even if the receiver generation has a numerically greater Lamport ID. This rule also applies when reset preserved an already-present receiver generation; the source need not first receive that generation.
