...
Restoration resumes previously emitted durable state and MUST NOT classify the restored graph as an empty-to-restored transition. Durable encoded cursors preserve their exact vector coordinates across restart and remain portable to every host without adoption or coverage rejection.
...
For every reset-touched NodeKey, reset retains one compact `ResetLineage(consumedThrough,correspondence)` on receiver authority. A present target stores it on the receiver freshness assertion anchored to the retained value origin; present-to-absent stores it on the real public delete. Absent-to-absent authors an internal reset-observation anchored to the existing delete or to explicit null absence when the receiver has no presence entry, with no public action. A source null observation itself makes the key reset-touched even without a source add/delete. The null anchor remains effective after delayed consumed source entries are unioned: entries through `consumedThrough` cannot manufacture presence, while post-cutoff generations/deletes participate in ordinary presence order. `consumedThrough` componentwise carries every prior receiver-anchor lineage vector, every source lineage selected by the journal-sync specification's canonical `applicableLineages(sourceJournal,K)`, validated source coverage, and every genuine retained public event coordinate for this NodeKey. Historical source carriers whose present/delete anchor does not resolve to the source raw head are excluded even when polling or exact-correspondence closure retains them. Thus reset-of-reset consumes the source snapshot's durable causal meaning without importing its journal or journalCoverage or manufacturing stronger future-union authority. On an unchanged repeat, the bookkeeping exclusion set is exactly the reset-lineage carriers whose `lineage_anchor` equals the receiver's current tagged anchor plus that present value-origin/delete anchor event itself; their carried vectors remain knowledge, but their own authoring coordinates are not recursively chased. Every lineage carrier from another historical anchor is genuine observed history and advances the new prefix, preventing it from self-reactivating after reset. Compaction retains null and delete-anchored observations even when delayed consumed history reappears, and retains the public coordinate maxima, so genuine delete/rematerialize/edit history remains distinguishable after restart. Bookkeeping on another key or anchor is never an exemption. A changed target includes its former anchor/carrier as consumed history. Reset does not install those coordinates in receiver journalCoverage, and source causal vectors never imply or transitively copy exact semantic correspondence.
