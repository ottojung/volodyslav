...
For every reset-touched NodeKey, reset retains one compact `ResetLineage(consumedThrough,consumedGeneration,consumedValueOrigin)` on receiver authority. A present target stores it on the receiver freshness assertion anchored to the retained value origin; an absent target stores it on the real public delete and uses null source IDs. `consumedThrough` is the componentwise closed-prefix union reset actually inspected across receiver and validated source snapshots, except that an unchanged repeated reset reuses its retained summary instead of chasing coordinates authored by that reset itself. Reset does not install those coordinates in receiver journalCoverage.
