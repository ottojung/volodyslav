...
Each lineage carrier is also an individual fallback assertion. Assertion N supersedes assertion O only when N's joined anchor cut covers O's actual anchor-presence coordinate (when non-null) and O's immutable carrier coordinate. Joined per-anchor vectors summarize absorption but do not collapse succession: `A1 -> B1 -> A2` selects A2 when A2 observed B1, regardless of B1 having observed historical A1. A currently inapplicable maximal assertion remains future-relevant because a later raw displacement inside its cut can make it applicable again. Compaction retains the maximal assertion antichain and the per-author coordinates of its anchors; assertions observed by a later assertion are safely subsumed. On an unchanged repeat, the bookkeeping exclusion set is exactly the reset-lineage carriers whose `lineage_anchor` equals the receiver's current tagged anchor plus that present value-origin/delete anchor event itself; their carried vectors remain knowledge, but their own authoring coordinates are not recursively chased. Every lineage carrier from another historical anchor is genuine observed history and advances the new prefix. Bookkeeping on another key or anchor is never an exemption. A changed target includes its former anchor/carrier as consumed history. Reset does not install those coordinates in receiver journalCoverage, and source causal vectors never imply or transitively copy exact semantic correspondence.
