...
Each directed database-version transition uses the `JournalFormatCodec` defined normatively by `incremental-graph-journal-migrations.md` §9a. Its totality, determinism, capability restrictions, target re-canonicalization, and global NodeKey-injectivity requirements are owned there and are not restated in this user contract.
