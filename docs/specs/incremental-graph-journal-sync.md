...
Before authoring any local semantic event, the receiver joins the source header causal knowledge:
...
and includes every source semantic event reference/context actually inspected if it is not already covered by the source header.

This observation allocates no event by itself.
...
Choose the greater `SemanticHead` authority by the total event-ID authority order.
...
- choose the greatest certificate event among certificates naming V;
- all copies of the same `ValueRef` must carry the same immutable origin context.
...
If both snapshots claim the same V, supported-state invariants guarantee the same semantic payload/timestamps; ordinary synchronization does not compare them to establish identity.
...
For every node whose candidate metadata is simply adopted, preserve the foreign semantic IDs/frontiers/certificate and record a receiver-local `AdoptEvent` only when receiver synchronization-relevant state actually changes.

For every normalization-created soft invalidation or tombstone, use the newly authored receiver event authority.
...
- selected semantic head authority for a node;
...
If A copies B's value V, A records a local adoption event but keeps V's original `ValueId` as the present head authority.
...
The same rule applies to adopted certificates, invalidation coordinates, and tombstones.
...
Pure candidate joining uses deterministic maxima/componentwise maxima and therefore repeated delivery of already represented positive authority cannot change the candidate again.
...
Both are negative authority and are authored after all authorities observed by the synchronization transaction. They introduce no new value or validation candidate.
