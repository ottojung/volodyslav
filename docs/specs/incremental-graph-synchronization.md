...
7. any persisted `JournalEntryId` or reference does not have a positive arbitrary-precision `JournalSequence`, any causal/coverage/clock coordinate is negative or noncanonical, the local coverage coordinate differs from `localJournalClock`, or local authoring failed to allocate a positive sequence above transaction-observed history; fresh local clock/coverage alone may be zero;
