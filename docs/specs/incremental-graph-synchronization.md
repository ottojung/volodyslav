...
For every reset anchor consumed from either snapshot, reset reads the effective cut formed by same-anchor carriers plus the exact `ResetAnchorCutSummary`. Any authored reset lineage carries that complete cut. In addition, controlled reset enumerates each source anchor represented by either a reset-lineage carrier or `resetAnchorCuts`, computes its effective cut from both representations, and joins it into the receiver archive by exact `(NodeKey,taggedAnchor)`. This includes anchors without a causal-maximal carrier and makes compacted and uncompacted source snapshots install identical absorption archives. The summaries remain non-event absorption metadata, do not enter `causalSummary`, and preserve anchor identity for delayed same-anchor union. Cuts from different anchors combine in a lineage only because this controlled-reset decision observed and consumed each anchor; journal projection never combines concurrent anchor cuts.
