...
RLV = canonical coordinate witness for each (key,receiver anchor,observed author)
RLC = canonical carrier for each exact (key,receiver value anchor,source generation,source origin)
```
...
Merge is ACI and physical survivors are uniquely determined. Generation initial freshness, reset validations, polling maxima, equal-time provenance, and delayed covered invalidates obey the same equality. Reset observation selection runs independently of presence. For every anchor/author coordinate, RLV retains the carrier with greatest coordinate, breaking equal-coordinate ties by full JournalEntryId; their semantic join is componentwise max. RLC separately retains every exact correspondence still relevant to retained candidates, with deterministic full-ID tie-breaking. It also retains internal absent observations, receiver anchors, and active post-cutoff witnesses. Thus concurrent vectors join without false sequence causality, exact equality evidence is not widened to a prefix, and absent compaction preserves future rematerialization.

For n semantic keys, r durable authors, and c distinct exact semantic reset correspondences that must remain recognizable to lagging replicas, polling/value evidence is O(nr); frontiers, lineage coordinate witnesses, and validation vectors are O(nr²); exact correspondence carriers are O(c); coverage is O(r). Total is `O(nr²+c+r)`, reducing to O(nr²+c) for n>0,r>=1. The c term is necessary: with fixed n=1,r=2, repeated equal reset against distinct rematerialization origins creates Ω(c) exact membership facts. A causal prefix cannot compress them without falsely certifying unrelated origins, and discarding them breaks lagging certified peers. n=0 with no represented historical key has empty journal and O(r) coverage.
