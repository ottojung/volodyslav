...
For assertions on the same NodeKey, define `resetSubsumes(N,O)` when `causallyBefore(O,N)` and N's `absorbsThrough` componentwise covers O's `absorbsThrough`. This relation permits O's fallback assertion to leave RL; it never transfers O's vector to N's anchor. RL is the global per-key antichain of assertions not subsumed by another assertion, across all tagged anchors, whether currently applicable or displaced. It is not one survivor per historical anchor. Concurrent assertions remain because neither causality nor absorption can be borrowed across them.
