...
Frontiers remain whole sets; compaction cannot combine partial validations. `HF` may contain an older same-author hard member when the all-mode head is soft. Value-specific barriers for losing heads remain only when required by an unconsumed iterator obligation, candidate provenance, or future union and never stale another origin.
...
## Iterator preservation (normative)

For any iterator progress vector `P`, fixed snapshot coverage `S`, and `NodeFilter`
`F`, iteration over `compact(J)` returns every required matching public
notification represented by `J` in the componentwise range `(P,S]` and then
advances progress to `S`. The N seed retains the greatest coordinate for every
`(author,NodeKey,publicAction)` obligation. A retained representative cannot be
replaced by an unmatched entry because NodeKey and action are part of its group.
Filtering is applied after representative selection, while consumption uses `S`
rather than the last retained or returned entry. Consequently empty filtered
results and holes left by compaction still consume the entire snapshot.
Compaction preserves writer coordinates and never rebases iterator progress.
