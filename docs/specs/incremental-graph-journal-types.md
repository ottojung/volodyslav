...
**NodeKey Presence Projection Theorem (NodeKey domain).** K is materialized iff causal `presenceHead(J,K)` is GenerationJournalEntry; it is absent iff the head is delete or undefined. A null explicit-absence anchor may suppress a raw presence event inside its consumed vector without another post-cutoff event. Actual generation/delete events above their author coordinates and generations activated by above-cut scoped events remain eligible; only actual generation/delete IDs order the result. History generation/delete/generation is valid. Polling history membership is not current presence.
