...
Canonical compaction persists exactly one joined `ResetAnchorCutSummary` per represented historical `(NodeKey,taggedAnchor)`. It may outlive every journal carrier currently using that anchor. The anchor identity is durable semantic information because an unseen concurrent carrier for that exact anchor may arrive later. The summary is non-assertion metadata: it has no event ID, causal context, occurrence time, public action, fallback vote, or correspondence. Its sole meaning is the same-anchor componentwise absorption join needed to reconstruct `anchorCut`; union joins summaries only with summaries and carriers of that exact tagged anchor. Repeated evidence for one exact anchor collapses componentwise, while the number of distinct represented anchor identities may grow independently of semantic keys, authors, and correspondences.
...
For a validated source snapshot S, `representedSourceAnchors(K)` is the union of the tagged anchors of all source reset-lineage carriers for K and all anchors named by `S.resetAnchorCuts` for K. For each represented A, `sourceEffectiveCut(K,A)` is the componentwise join of every source carrier's `absorbsThrough` whose tagged anchor is A and `S.resetAnchorCuts[K,A]` when present. Controlled reset componentwise joins every `sourceEffectiveCut(K,A)` into receiver `resetAnchorCuts[K,A]`. This applies when A has no causal-maximal carrier, is displaced, is not the new receiver anchor, or exists only in uncompacted carriers. These summaries remain non-event absorption metadata and do not enter `causalSummary`. Archive preservation is additional future-union state: it does not replace the requirement that each reset-authored lineage carry the effective cuts it consumes. Controlled reset imports neither source journal entries nor source `journalCoverage`.

Before no-op classification, controlled reset computes `resultingResetAnchorCuts` by taking receiver `resetAnchorCuts` and componentwise joining every `sourceEffectiveCut(K,A)` by exact `(NodeKey,taggedAnchor)`. A reset is settled only when the receiver already represents the target SemanticGraph, required freshness/proof authority, required `ResetCorrespondence`, every effective cut consumed into reset-authored lineage, required causal knowledge, and the complete `resultingResetAnchorCuts`. Archive growth alone is a valid committed reset transition: it authors no journal event and leaves graph, `localJournalCounter`, timestamps, and otherwise unchanged causal authority untouched. Repeating the reset after that join is completely silent and byte-for-byte unchanged.
